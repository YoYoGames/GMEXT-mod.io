 ...
 * @param {struct} optionals A struct containing the following info: 
 * 
 ...
 * 
 * [[NOTE: To use this endpoint you will need to set up some additional settings prior to being able to authenticate Xbox Live users. For these instructions you should [contact mod.io](mailto:developers@mod.io).]]
 * 
 * @param {string} xbox_token 
 * @param {struct} optionals
 * 
 * * ${string} email The user's email address (optional but recommended to help users recover lost accounts). If supplied, and when the respective user does not have an email registered for their account we will send a confirmation email to confirm they have ownership of the specified email. This functionality is also available at a later time via the [Link an Email](https://docs.mod.io/#link-an-email) endpoint. NOTE: If the user already has an email on record with us, this parameter will be ignored. This parameter should also be URL encoded before the request is sent.
 * @param {int64} date_expires The Unix timestamp of the date on which the returned token will expire. The value cannot be higher than the default, value which is a common year (unix timestamp + 31536000 seconds). Using a token after its expiry time has elapsed will result in a 401 Unauthorized response.
 ...
 * [[NOTE: If the user already has an email on record with us, this parameter will be ignored. This parameter should also be URL encoded before the request is sent.]]
 * 
 * @param {int64} date_expires The Unix timestamp of the date on which the returned token will expire. The value cannot be higher than the default value which is a week (unix timestamp + 604800 seconds). Using a token after its expiry time has elapsed will result in a `401 Unauthorized` response.
 * @param {boolean} terms_agreed This *must* be set to `false` unless you have collected the [users agreement](https://docs.mod.io/#terms) prior to calling this endpoint, in which case it can be set to `true` and will be recorded.
 * 
 ...
 * <br />
 * 
 * This function requests a security code for a user, identified by their e-mail which can then be exchanged for an access token. To use this functionality you **must** use your game's api_key from your game's profile on mod.io. A Successful request will return a Message Object.
 * 
